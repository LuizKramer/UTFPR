...
int conta_chaves_folha(NodeB * tree){
    int i;
    int a = 0;
    ...
    if (tree != NULL)
    {
        ...
        {
            a += conta_chaves_folha(tree->filhos[i]);
        }
    }
    else{
        return 0;
...
int media_folhas(NodeB * tree){
    return soma_folhas(tree) / conta_chaves_folha(tree);
}
